...
export function calcCartTotals(cart: CartTotalsInput, cashRounding: number): CartTotals {
  ...
  // P1: kẹp CK bill vào [0, tiền hàng] — khớp guard server 0028. Client gửi discount
  // tùy ý qua RPC; server clamp lại nên local phải clamp trước để preview = receipt.
  discount = Math.min(Math.max(0, Math.round(discount || 0)), subtotal);
  ...
  const shipping =
    cart.shipping_type === 'percent'
      ...
      : Math.max(0, Math.round(cart.shipping_fee || 0));
  const vat_percent = Math.min(Math.max(0, cart.vat_percent || 0), 100);

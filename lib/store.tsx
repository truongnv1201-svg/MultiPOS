...
import { useRealtimeSync, type RealtimeStatus } from './store/realtime-sync';
...
function StoreInner({ children }: { children: React.ReactNode }) {
  ...
  const refreshAllRealtime = useCallback(async () => {
  ...
  }, [refreshCatalog, refreshCustomers, refreshOrders, refreshCashbook]);
  ...
  // P3: có mạng -> kéo catalog + KH + đơn hàng + sổ quỹ từ server
  ...
  // P2: memoize facade value — StoreInner re-render không đổi tham chiếu value khi không
  // có slice nào đổi, tránh re-render dây chuyền toàn bộ consumer useStore().
  const value = useMemo(
    () => ({
      currentScreen,
      setCurrentScreen,

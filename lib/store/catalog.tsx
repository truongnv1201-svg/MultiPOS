...
export function CatalogProvider({ children }: { children: React.ReactNode }) {
  ...
  // P3/Realtime: tải danh sách khách hàng từ server (kèm công nợ thực tế)
  const refreshCustomers = useCallback(async (): Promise<boolean> => {
    if (!supa) return false;
    try {
      const { data, error } = await supa.from('customers').select('*').order('name');
      if (error || !data) return false;
      const mapped: Customer[] = (data as any[]).map((row) => ({
        id: row.id,
        code: row.code,
        name: row.name,
        phone: row.phone,
        address: row.address ?? undefined,
        group: row.customer_group || 'retail',
      ...
      try {
        ...
        await db.customers.bulkAdd(mapped);
      } catch {
        /* best-effort */
      }
      return true;
  ...
  const addProduct = useCallback(
    async (data: Omit<Product, 'id' | 'sku'> & { sku?: string }): Promise<Product> => {
      if (supa && profile?.role !== 'admin' && profile?.role !== 'manager') {
        toast('Chỉ Admin/Quản lý được thêm hàng hóa!');
  ...
  const updateProduct = useCallback(async (id: string, updates: Partial<Product>) => {
    if (supa && profile?.role !== 'admin' && profile?.role !== 'manager') {
      toast('Chỉ Admin/Quản lý được sửa hàng hóa/giá vốn!');
      return;
    }

...
export function HrmProvider({ children }: { children: React.ReactNode }) {
  ...
  const requireHrmManager = useCallback(
    (action: string): boolean => {
      ...
      if (profile?.role !== 'admin' && profile?.role !== 'manager') {
        toast(`Chỉ Admin/Quản lý được ${action}!`);
        return false;
      }
      return true;
    },
  ...
  const saveEmployee = useCallback(
    async (input: EmployeeInput): Promise<boolean> => {
      ...
      if (!input.full_name.trim()) {
        toast('Nhập tên nhân viên!');
        return false;
      }
      if (!isOnline || !supa || !user) {
        toast('Cần Online mới thêm/sửa nhân viên được!');
        return false;
      }
      ...
      try {
        ...
        toast(`Lưu nhân viên thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const createLoginAccount = useCallback(
    async (
      ...
      if (!isOnline || !supa || !user) {
        toast('Cần Online mới tạo tài khoản được!');
        return null;
      }
      try {
        ...
        if (!res.ok) {
          toast(json.error || 'Tạo tài khoản thất bại.');
          return null;
        }
  ...
  const createEmployeeWithAccount = useCallback(
    async (emp: EmployeeInput, acct: { password: string; role: string }): Promise<boolean> => {
      ...
      if (profile?.role !== 'admin' && acct.role !== 'cashier' && acct.role !== 'worker') {
        toast('Quản lý chỉ được tạo tài khoản Thu ngân / Thợ!');
        return false;
      }
      if (!isOnline || !supa || !user) {
        toast('Cần Online mới thêm nhân viên được!');
        return false;
      }
      try {
        const code = nextEmployeeCode(employees);
  ...
  const createAccountForEmployee = useCallback(
    async (employeeId: string, acct: { password: string; role: string }): Promise<boolean> => {
      ...
      try {
        ...
        toast(`Gắn tài khoản thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const resetEmployeePassword = useCallback(
    async (employeeId: string, newPassword: string): Promise<boolean> => {
      ...
      const emp = employees.find((e) => e.id === employeeId);
      if (!emp?.user_id) {
        toast('Hồ sơ này chưa có tài khoản đăng nhập!');
        return false;
      }
      ...
      if (!isOnline || !supa) {
        toast('Cần Online mới đặt lại được!');
        return false;
      }
      try {
        const { data: sess } = await supa.auth.getSession();
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          toast(json.error || 'Đặt lại mật khẩu thất bại.');
          return false;
        }
        toast(`Đã đặt lại mật khẩu cho ${emp.code} — ${emp.full_name}.`);
        return true;
      } catch (err: any) {
        toast(vietnamizeError(err));
        return false;
  ...
  const addAdvance = useCallback(
    async (input: {
      ...
      if (!emp || emp.status !== 'active') {
        toast('Chọn nhân viên đang làm việc!');
        return false;
      }
      const amount = Math.max(0, Math.round(Number(input.amount) || 0));
      if (amount <= 0) {
        toast('Nhập số tiền tạm ứng!');
        return false;
      }
      ...
      if (!isOnline || !supa || !user) {
        toast('Cần Online mới ghi tạm ứng được!');
        return false;
      }
      const month = input.advance_date.slice(0, 7);
      if (payrollLocks.includes(month)) {
        toast(`Tháng ${month} đã chốt lương — không ghi tạm ứng được!`);
        return false;
      }
      try {
        const { data, error } = await supa
        ...
        toast(`Ghi tạm ứng thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const deleteAdvance = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      if ((run && run.status !== 'draft') || payrollLocks.includes(adv.month)) {
        toast(`Tháng ${adv.month} đã chốt/chi lương — không xóa tạm ứng được!`);
        return false;
      }
      if (!isOnline || !supa) {
        toast('Cần Online mới xóa được!');
        return false;
      }
      if (!(await confirmDialog(`Xóa lần tạm ứng ${adv.amount.toLocaleString('vi-VN')}đ của ${adv.employee_name || ''} ngày ${adv.advance_date}? (Xóa luôn phiếu chi ${adv.cashbook_code || ''})`, { danger: true }))) return false;
      try {
        ...
        toast(`Xóa tạm ứng thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const addAdvanceVoucher = useCallback(
    async (input: { employee_id: string; amount: number; fund_type: 'cash' | 'bank'; note?: string }): Promise<boolean> => {
      ...
      if (currentShift.status !== 'open') {
        toast('Ca đã đóng! Mở ca mới trước khi lập phiếu.');
        return false;
      }
      const emp = employees.find((e) => e.id === input.employee_id);
      if (!emp || emp.status !== 'active') {
  ...
  const setEmployeeStatus = useCallback(
    async (id: string, status: 'active' | 'inactive'): Promise<boolean> => {
      ...
      if (!isOnline || !supa) {
        toast('Cần Online mới đổi trạng thái được!');
        return false;
      }
      if (status === 'inactive' && !(await confirmDialog('Cho nhân viên này nghỉ việc? (Giữ lại lịch sử công/lương)', { danger: true }))) return false;
      try {
        ...
        toast(`Đổi trạng thái thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const setMonthLock = useCallback(
    async (month: string, locked: boolean): Promise<boolean> => {
      ...
      try {
        ...
        toast(`Chốt/mở bảng công thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const markDay = useCallback(
    async (input: MarkDayInput): Promise<boolean> => {
      ...
      if (lockMonth && payrollLocks.includes(lockMonth)) {
        toast(`Bảng công tháng ${lockMonth} đã chốt lương — liên hệ Admin để mở khóa!`);
        return false;
      }
      ...
      if (found) {
        ...
        if (found.synced && isUuid(found.id) && canWriteServer && supa) {
          ...
          if (error) {
            toast(`Lưu server thất bại: ${vietnamizeError(error)}`);
            return false;
          }
          updated.synced = true;
        }
  ...
  const clearDay = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      if (rec.synced && isUuid(id)) {
        if (!supa || !isOnline) {
          toast('Dòng này đã đồng bộ server — cần Online mới xóa được!');
          return false;
        }
        const { error } = await supa.from('attendance_days').delete().eq('id', id);
        if (error) {
          toast(`Xóa server thất bại: ${vietnamizeError(error)}`);
          return false;
        }
      }
  ...
  const generatePayroll = useCallback(
    async (month: string): Promise<boolean> => {
      ...
      if (!isOnline || !supa || !user) {
        toast('Cần Online mới lập bảng lương được!');
        return false;
      }
      const existing = payrollRuns.find((r) => r.month === month);
      if (existing?.status === 'paid') {
        toast(`Tháng ${month} đã chi lương — không tạo lại được!`);
        return false;
      }
      if (existing && !(await confirmDialog(`Tạo lại bảng lương tháng ${month}? (Xóa bản hiện tại)`, { danger: true }))) return false;
      try {
        ...
        toast(`Lập bảng lương thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const lockMonth = useCallback(
    async (month: string): Promise<boolean> => {
      ...
      if (!isOnline || !supa || !user) {
        toast('Cần Online mới chốt được!');
        return false;
      }
      const existing = payrollRuns.find((r) => r.month === month);
      if (existing?.status === 'paid') {
        toast(`Tháng ${month} đã chi lương — không chốt lại được!`);
        return false;
      }
      if (!(await confirmDialog(`Chốt tháng ${month}? (Dựng lại bảng lương từ số mới nhất, khóa chấm công + chốt lương)`, { danger: true }))) return false;
      try {
        const runId = await buildDraftPayroll(month);
        if (!runId) return false;
        ...
        toast(`Chốt tháng thất bại: ${vietnamizeError(err)}`);
        return false;
      }
    },
  ...
  const reopenPayroll = useCallback(
    async (runId: string): Promise<boolean> => {
      ...
      if (!run || run.status !== 'finalized') {
        toast('Chỉ mở lại được bảng đã chốt (chưa chi)!');
        return false;
      }
      if (!(await confirmDialog(`Mở lại bảng lương tháng ${run.month} để sửa? (Mở khóa chấm công)`, { danger: true }))) return false;
      ...
      if (error) {
        toast(`Mở lại thất bại: ${vietnamizeError(error)}`);
        return false;
      }
      await refreshHrm();
      return true;
  ...
  const payPayroll = useCallback(
    async (runId: string, fund: 'cash' | 'bank'): Promise<boolean> => {
      ...
      const run = payrollRuns.find((r) => r.id === runId);
      if (!run || run.status !== 'finalized') {
        toast('Chỉ chi được bảng đã chốt!');
        return false;
      }
      const items = payrollItems.filter((i) => i.run_id === runId && !i.paid && i.net > 0);
      if (items.length === 0) {
        toast('Không còn dòng lương nào cần chi!');
        return false;
      }
      if (!(await confirmDialog(`Chi lương tháng ${run.month} cho ${items.length} người (${formatPayrollTotal(items)}) qua ${fund === 'cash' ? 'Tiền mặt' : 'Ngân hàng'}?`, { danger: true }))) return false;
      try {
        ...
        toast(`Chi lương thất bại: ${vietnamizeError(err)}`);
        await refreshHrm();
        return false;
      }
    },

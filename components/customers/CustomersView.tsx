...
export function CustomersView() {
  ...
  const handleSyncDebts = async () => {
    ...
    try {
      ...
      if (updated > 0 || skipped > 0) {
        toast(`Đồng bộ công nợ xong: ${updated} KH cập nhật theo server${skipped > 0 ? `, ${skipped} KH chưa liên kết nên bỏ qua` : ''}.`, 'success');
      }
    } finally {
  ...
  const handleConfirmCollect = async (e: React.FormEvent) => {
    ...
    toast(`Đã thu ${formatVND(collectAmount)} tiền nợ của ${selectedCustomer.name} thành công!`, 'success');
  };
  ...
  const handleCreateCustomer = async (e: React.FormEvent) => {
    ...
    setName('');
    setPhone('');
    toast('Thêm khách hàng mới thành công!', 'success');
  };

  return (
    <div id="customers-view" className="flex-1 flex flex-col h-[calc(100vh-56px)] bg-slate-100 overflow-hidden">
      {/* Top Header */}
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 p-4 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-slate-100 min-w-0">
          ...
          <div className="px-3 py-1.5 bg-slate-100/70 border-b border-slate-200 flex items-center justify-between text-[11px] font-medium text-slate-600">
            <span>
              Tìm thấy <strong className="text-slate-900 font-mono tnum">{filteredCustomers.length}</strong> khách hàng
            </span>
            <span>
              Tổng công nợ nhóm này:{' '}
              <strong className="font-mono tnum text-rose-600 font-bold">{formatVND(totalFilteredDebt)}</strong>
            </span>
          </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedCustomers.length === 0 ? (
                  <tr>
                    <td colSpan={7}>
                      <EmptyState message="Không tìm thấy khách hàng nào phù hợp với bộ lọc." />
                    </td>
                  </tr>
                ) : (
                  paginatedCustomers.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 text-right font-mono tnum text-slate-500">
            ...
            <div className="space-y-4 text-xs">
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs space-y-3">
                ...
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-lg space-y-1">
                  ...
                  <div className="text-xl font-extrabold text-rose-600 font-mono tnum">
            ...
            <div className="p-4 space-y-3 text-xs">
              <div className="p-2.5 bg-slate-50 rounded border border-slate-200">
                ...
                <div className="text-slate-500">
                  Dư nợ hiện tại: <strong className="text-rose-600 tnum">{formatVND(selectedCustomer.current_debt)}</strong>
                </div>
              </div>

              <Field label="Số tiền thu nợ (đ)" required>
                <NumberInput
                  required
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button
                    ...
                    variant={collectMethod === 'cash' ? 'primary' : 'secondary'}
                    className="flex-1"
                  >
                    Tiền mặt (Két quầy)
                  </Button>
                  <Button
                    ...
                    variant={collectMethod === 'transfer' ? 'primary' : 'secondary'}
                    className="flex-1"
                  >
                    Chuyển khoản VietQR
                  </Button>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3">
          ...
          >
            ...
            <div className="p-4 space-y-3 text-xs">
              <Field label="Tên khách hàng / Xưởng nhôm kính" required>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <Field label="Hạn mức nợ cho phép (đ)">
                <NumberInput
                  value={debtLimit}

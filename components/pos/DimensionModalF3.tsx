...
import { HOLE_PRICE, CORNER_PRICE } from '@/lib/catalog-constants';
...
function DimensionModalDialog({ item, isNew, onClose }: DialogProps) {
  ...
  const handleRemoveRow = (id: string) => {
    if (rows.length <= 1) {
      toast('Đơn hàng m² bắt buộc có ít nhất 1 dòng quy cách!', 'error');
      return;
    }
  ...
  const handleSave = useCallback(() => {
    ...
    if (badIdx >= 0) {
      toast(`Dòng ${badIdx + 1} chưa đủ kích thước! Dài, Rộng và Số tấm phải lớn hơn 0.`, 'error');
      return;
    }

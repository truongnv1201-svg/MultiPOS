...
export function ShiftModalF12() {
  ...
  const handleConfirmClose = async () => {
    if (needLogin) {
      toast('Vui lòng đăng nhập thu ngân trước khi kết ca!', 'error');
      setLoginOpen(true);
      return;
    }
    if (isLocked) {
      toast('LỖI OFF-ERR-01: Thiết bị đang Offline hoặc còn đơn hàng ngoại tuyến chưa đồng bộ!', 'error');
      return;
    }
    setIsClosing(true);
    try {
      ...
      if (ok) {
        toast('Đóng ca làm việc thành công! Biên bản kết ca đã được khóa sổ vĩnh viễn.', 'success');
      }
    } finally {
      setIsClosing(false);
    }
  ...
  const handleStartNewShift = async () => {
    if (needLogin) {
      toast('Vui lòng đăng nhập trước khi mở ca!', 'error');
      setLoginOpen(true);
      return;
    }
    await openNewShift(newShiftStartingCash);
    toast('Bắt đầu ca làm việc mới thành công!', 'success');
            ...
            <>
              ...
              <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200 text-xs space-y-2 font-mono">
                <div className="flex justify-between text-slate-600">
                  ...
                  <span className="font-bold text-slate-900">{currentShift.cashier_name}</span>
                </div>
                <div className="flex justify-between text-slate-600">

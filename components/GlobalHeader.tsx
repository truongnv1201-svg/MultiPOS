...
export function GlobalHeader() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const blockRestricted = () => {
        toast('Tài khoản thu ngân không có quyền vào phân hệ này!', 'error');
        return;
      };
      const blockNotAdmin = () => {
        toast('Chỉ Admin/Quản lý được vào Cài đặt (Quản lý chỉ quản lý nhân viên, cấu hình hệ thống vẫn chỉ Admin)!', 'error');
        return;
      };
  ...
  return (
    <header
      id="global-header"
      className="w-full h-14 bg-slate-900 text-white z-40 px-3 border-b border-slate-800 flex items-center justify-between select-none shadow-md shrink-0"
    >
      ...
      {/* RIGHT: Status, Branch, Shift, Actions */}
      <div className="flex items-center gap-2">
        ...
        {/* Offline / Online Toggle with Pending Badge */}
          ...
          >
            ...
            {pendingQueue.length > 0 && (
              <span
                ...
                className="ml-1 px-1.5 py-0.2 text-[10px] font-bold bg-amber-500 text-slate-900 rounded-full cursor-pointer hover:bg-amber-400"
        ...
        {/* P6: Auth — 1 nút user duy nhất hiện tên (không trùng với nút ca) */}
        {user ? (
          <button
            id="header-user-btn"
            onClick={async () => {
              if (await confirmDialog(`Đăng xuất khỏi ${profile?.full_name || user.email}?`)) signOut();
            }}
            className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-800 rounded-md text-[11px] text-emerald-300 transition-colors"
            title={`${profile?.full_name || user.email} (${profile?.role === 'admin' ? 'Quản trị viên' : profile?.role || '...'}) — Click để đăng xuất`}
          >
        ...
        <button
          ...
          onClick={() => {
            if (profile?.role !== 'admin' && profile?.role !== 'manager') {
              toast('Chỉ Admin/Quản lý được vào Cài đặt!', 'error');
              return;
            }
            setCurrentScreen('settings');
          }}

...
export function CashbookView() {
  ...
  const handleCreateVoucher = async (e: React.FormEvent) => {
    ...
    if (isAdvanceVoucher) {
      if (!isManager) {
        toast('Chỉ Admin/Quản lý được ghi tạm ứng lương!', 'error');
        return;
      }
      if (!advanceEmpId) {
        toast('Chọn nhân viên nhận tạm ứng để đưa vào bảng lương!', 'error');
        return;
      }
      ...
      setNote('');
      setAdvanceEmpId('');
      toast('Đã ghi tạm ứng — tự trừ vào bảng lương tháng này.', 'success');
      return;
    }
    ...
    setPartnerName('');
    setNote('');
    toast(`Đã lập ${voucherType === 'receipt' ? 'Phiếu thu' : 'Phiếu chi'} thành công!`, 'success');
  };

  return (
    <div id="cashbook-view" className="flex-1 flex flex-col h-[calc(100vh-56px)] bg-slate-100 overflow-hidden">
      {/* Header */}
      ...
      <div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white border-b border-slate-200">
        <div className="bg-blue-50/80 p-3.5 rounded-xl border border-blue-200">
          ...
          <div className="text-xl font-extrabold text-blue-900 font-mono tnum">
        ...
        <div className="bg-amber-50/80 p-3.5 rounded-xl border border-amber-200">
          ...
          <div className="text-xl font-extrabold text-amber-900 font-mono tnum">
        ...
        <div className="bg-emerald-50/80 p-3.5 rounded-xl border border-emerald-200">
          ...
          <div className="text-xl font-extrabold text-emerald-900 font-mono tnum">
      ...
      <div className="flex-1 flex flex-col p-4 overflow-hidden">
        ...
        <div className="px-3 py-1.5 bg-slate-100/70 border-b border-slate-200 flex flex-wrap items-center justify-between text-[11px] font-medium text-slate-600 gap-2">
          <span>
            Tìm thấy <strong className="text-slate-900 font-mono tnum">{filteredEntries.length}</strong> bút toán
          </span>
          <div className="flex items-center gap-4">
            <span>
              Tổng thu:{' '}
              <strong className="font-mono tnum text-emerald-700 font-bold">+{formatVND(filteredReceiptTotal)}</strong>
            </span>
            <span>
              Tổng chi:{' '}
              <strong className="font-mono tnum text-rose-700 font-bold">-{formatVND(filteredExpenseTotal)}</strong>
            </span>
            <span>
              Thu ròng:{' '}
              <strong
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {paginatedEntries.length === 0 ? (
                <tr>
                  <td colSpan={7}>
                    <EmptyState message="Không tìm thấy chứng từ thu chi nào phù hợp với bộ lọc." />
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-2.5 px-3">
                        <Badge tone={e.fund_type === 'cash' ? 'amber' : 'emerald'}>
                      ...
                      <td className="py-2.5 px-3 text-right font-mono font-bold tnum text-sm">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3">
          ...
          >
            ...
            <div className="p-4 space-y-3 text-xs">
              ...
              <Field label="Người nộp / nhận tiền">
                {isAdvanceVoucher ? (
                  <>
                    <select
                      value={advanceEmpId}

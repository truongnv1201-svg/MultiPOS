...
export function HRMView() {
  ...
  return (
    <div id="hrm-view" className="flex-1 flex flex-col h-[calc(100vh-56px)] bg-slate-100 overflow-hidden">
      <div className="h-14 px-4 bg-white border-b border-slate-200 flex items-center gap-3 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2 min-w-0">
          <div className="flex gap-1 overflow-x-auto" role="tablist" aria-label="Chức năng nhân sự">
            {TABS.map((t) => (
              <Button
                ...
                variant={subTab === t.id ? 'primary' : 'secondary'}
          ...
          {hrmLoading && (
            <span className="hidden md:flex items-center gap-1.5 text-[11px] text-slate-400 shrink-0" role="status">
          ...
          {payrollLocks.includes(currentMonthKey()) && (
            <Badge tone="amber" className="hidden sm:inline-flex shrink-0">
...
function AttendancePayrollTab({
  ...
  const handleExportPayroll = () => {
    if (!run || items.length === 0) {
      toast('Chưa có bảng lương để xuất!', 'error');
      return;
    }
  ...
  const handlePrintPayroll = () => {
    if (!run || items.length === 0) {
      toast('Chưa có bảng lương để in!', 'error');
      return;
    }
    printTable({
    ...
    <div className="space-y-4">
      ...
      <section aria-label={`Bảng lương tháng ${monthKey}`}>
        <div className={`${CARD} overflow-hidden`}>
          {/* Header — tiêu đề + nút gộp chung khối với bảng */}
          <div className="px-3 py-2 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center gap-2">
            ...
            {isManager && (
              ...
              <Button
                ...
                onClick={async () => {
                  if (run && run.status === 'paid') {
                    toast(`Tháng ${monthKey} đã chi lương — không tạo lại được!`, 'error');
                    return;
                  }
                  setBusy(true);
                  try {
              ...
              {run && run.status === 'finalized' && (
                <>
                  <Button onClick={() => onReopen(run.id)} variant="ghost">
                  ...
                  <Button onClick={() => onPay(run.id, fund)} variant="success">
          ...
          <PayrollTab
            ...
            bare
          />
          <p className="px-3 py-2 border-t border-slate-200 bg-slate-50/60 text-xs text-slate-400 flex items-center gap-1.5">
            <Filter className="w-3.5 h-3.5 shrink-0" />
            Bảng lương chỉ đọc — tổng hợp từ chấm công và tạm ứng. Bấm Chốt tháng để khóa cả công và lương một lần; chi lương sinh phiếu chi trong sổ quỹ.
          </p>
        </div>
      </section>
    </div>
        ...
        <div className="space-y-3">
          ...
          <div className={`${CARD} p-4`}>
            <div className="flex items-center gap-2">
              ...
              {run && (
                <Badge
                  tone={run.status === 'paid' ? 'emerald' : run.status === 'finalized' ? 'blue' : 'slate'}
                  className="ml-auto"
                >
            ...
            <Button onClick={() => setCurrentScreen('payroll')} variant="primary" className="w-full mt-3">
...
function StaffTab({
  ...
  const handleImportExcel = async (file: File) => {
    ...
    try {
      ...
      toast(`Nhập xong: ${created} tạo mới, ${updated} cập nhật (hồ sơ chưa gắn tài khoản — vào Sửa để tạo).${errors.length > 0 ? `\nLỗi:\n${errors.join('\n')}` : ''}`, errors.length > 0 ? 'error' : 'success');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-3">
      <div className={`${CARD} p-2.5 flex flex-wrap items-center gap-2`}>
        <div className="flex items-center gap-1.5 bg-slate-100 rounded-md px-2 h-8 flex-1 min-w-[180px] focus-within:border-blue-500 border border-transparent transition">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="text-slate-400 hover:text-slate-600 cursor-pointer" aria-label="Xóa tìm kiếm">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        ...
        />
        {isManager && (
          <Button onClick={() => setEditing('new')} variant="primary" className="ml-auto">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3" role="dialog" aria-modal="true" aria-label={initial ? 'Sửa hồ sơ nhân viên' : 'Thêm nhân viên'}>
      ...
      <div className="relative bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md max-h-[92vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-4 py-3 bg-slate-900 text-white flex items-center gap-2.5 shrink-0">
          ...
          <Button onClick={onClose} variant="ghost-light" size="icon" className="ml-auto" aria-label="Đóng">
            <X className="w-4 h-4" />
          </Button>
        </div>
                  ...
                  <div className="flex gap-2 mt-1">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        if (!(await confirmDialog(`Đổi mật khẩu ${initial.code}? NV dùng mật khẩu mới từ lần đăng nhập sau.`, { danger: true }))) return;
                        setPwBusy(true);
                        try {
            ...
            <section>
              ...
              >
                ...
                {liveStatus === 'active' ? (
                  <Button
                    onClick={async () => {
                      if (!(await confirmDialog(`Cho ${initial.code} — ${initial.full_name} nghỉ việc? (Giữ lại lịch sử công/lương)`, { danger: true }))) return;
        ...
        <div className="px-3 py-2.5 border-t border-slate-200 flex justify-end gap-2 shrink-0 bg-slate-50">
          ...
          <Button onClick={handleSave} disabled={busy} variant="primary">
...
function AttendanceTab({
  ...
  const fillRowPresent = async () => {
    ...
    if (missing.length === 0) {
      toast(`${selEmp.full_name} đã đủ công các ngày làm việc trong tháng.`);
      return;
    }
    if (!(await confirmDialog(`Chấm “Đi làm” cho ${missing.length} ngày trống của ${selEmp.full_name} trong T${month}?`))) return;
    setFilling(true);
    try {
  ...
  return (
    <div className="space-y-3">
      <div className={`${CARD} overflow-hidden`}>
        {/* Header — tiêu đề + filter + Excel/In gộp chung một hàng */}
        <div className="px-3 py-2 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center gap-2">
          <h4 className="text-sm font-extrabold text-slate-800">Chấm công tháng {monthKey}</h4>
          {locked && (
            <Badge tone="amber" title="Đã chốt ở mục lương bên dưới — bấm Mở lại để chấm/sửa">
              <Lock className="w-3 h-3" /> Đã chốt {monthKey}
            </Badge>
          )}
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1">
              ...
              <span className="font-bold text-slate-800 min-w-[86px] text-center text-xs tnum">
                T{month}/{year}
              </span>
              <Button
                onClick={() => {
            ...
            <div className="flex items-center gap-1.5 bg-white rounded-md px-2 h-8 w-44 border border-slate-300 focus-within:border-primary transition">
              <Search className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm nhân viên…" aria-label="Tìm nhân viên" className="bg-transparent outline-none text-xs w-full" />
            </div>
            <label className="text-xs font-semibold text-slate-700 flex items-center gap-1.5 cursor-pointer select-none">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs text-slate-500 px-3 py-2 border-t border-slate-200 bg-slate-50/60">
      ...
      {sel && selEmp && (
        <div className={`${CARD} p-3.5`}>
          ...
          <div className="flex flex-wrap items-end gap-2 mt-3">
            ...
            <Field label="Ghi chú" htmlFor="att-note" className="flex-1 min-w-[180px]">
              <input
                id="att-note"
                value={note}
                ...
                className="mt-1 w-full h-8 border border-slate-300 rounded-md px-2.5 text-xs disabled:opacity-50"
              />
            </Field>
            {isManager && !locked && (
              <>
                ...
                {selRec && (
                  <Button
                    onClick={async () => {
                      if (await confirmDialog(`Xóa công ${selEmp.full_name} ngày ${sel.iso}?`, { danger: true })) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3" role="dialog" aria-modal="true" aria-label={`Tạm ứng lương tháng ${month}`}>
      ...
      {isManager && !locked && (
        <div className="flex flex-wrap items-end gap-2 mt-2.5 bg-slate-50 border border-slate-200 rounded-lg p-2.5">
          <Field label="Nhân viên" required className="min-w-[170px]">
            <select value={empId} onChange={(e) => setEmpId(e.target.value)} className="border border-slate-300 rounded-md px-2 h-8 text-xs bg-white w-full">
              <option value="">— Chọn —</option>
              {actives.map((e) => (
          ...
          <Field label="Số tiền (đ)" required className="w-32">
            <NumberInput value={amount} min={0} onChange={setAmount} placeholder="0" className="border border-slate-300 rounded-md px-2 h-8 text-xs tnum w-full" />
          </Field>
          <Field label="Quỹ chi">
            <select value={fund} onChange={(e) => setFund(e.target.value as 'cash' | 'bank')} className="border border-slate-300 rounded-md px-2 h-8 text-xs bg-white w-full">
              <option value="cash">Tiền mặt</option>
              <option value="bank">Ngân hàng</option>
            </select>
          </Field>
          <Field label="Ngày ứng">
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="border border-slate-300 rounded-md px-2 h-8 text-xs tnum w-full" />
          </Field>
          <Field label="Ghi chú" className="flex-1 min-w-[140px]">
            <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Lý do ứng…" className="border border-slate-300 rounded-md px-2 h-8 text-xs w-full" />
          </Field>
          <Button
        ...
        <ul className="divide-y divide-slate-100 max-h-64 overflow-y-auto border border-slate-100 rounded-lg">
          {list.map((a) => (
            <li key={a.id} className="px-2.5 py-1.5 flex items-center gap-2 text-xs hover:bg-slate-50">
              ...
              {a.cashbook_code && <span className="ml-auto font-mono tnum text-[10px] text-slate-400 whitespace-nowrap">{a.cashbook_code}</span>}
              {isManager && !locked && (
                <Button
                  onClick={() => onDelete(a.id)}
                  variant="ghost"
                  size="icon"
                  className={a.cashbook_code ? '' : 'ml-auto'}
...
function PayrollTab({
  ...
  advOpen: boolean;
  onCloseAdv: () => void;
  bare?: boolean; // true: render trần (table/empty) để parent bọc card chung + header/note
}) {
  return (
    <div className="space-y-3">
      {!run ? (

    ...
    >
      ...
      <div className="flex items-center gap-3">
        <span className="text-slate-600 font-medium">
          Hiển thị{' '}
          <strong className="text-slate-900 tnum">
            {startItem}-{endItem}
          </strong>{' '}
          trên tổng số{' '}
          <strong className="text-slate-900 tnum">{totalItems}</strong>{' '}
          {itemName}
        </span>
          ...
          {getPageNumbers().map((p, idx) => {
            ...
            return (
              <button
                ...
                className={`min-w-7 h-7 px-1.5 text-xs font-semibold rounded transition-colors cursor-pointer ${
                  isActive

...
export function ProductsView() {
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    setName('');
    toast('Thêm mới sản phẩm thành công!', 'success');
  };

  return (
    <div id="products-view" className="flex-1 flex flex-col h-[calc(100vh-56px)] bg-slate-100 overflow-hidden">
      {/* Top Header */}
      ...
      <div className="p-2.5 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          {[
            ...
            <Button
              key={tab.id}
              variant={typeFilter === tab.id ? 'primary' : 'secondary'}
              onClick={() => {
                ...
                paginatedProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-right font-mono tnum font-bold text-blue-600">
                        {formatVND(p.retail_price)}
                      </td>
                      <td className="py-2.5 px-3 text-right font-mono tnum text-slate-600">
          ...
          >
            <div className="px-4 py-3 bg-slate-900 text-white flex items-center justify-between">
              <h3 className="font-bold text-sm flex items-center gap-2">
                ...
                Thêm Hàng Hóa Mới (Tự Sinh Mã SP0000xx)
              </h3>
              <Button
                variant="ghost-light"
                size="icon"
            ...
            <div className="p-4 space-y-3 text-xs">
              ...
              {productType === 'area' && (
                <div className="grid grid-cols-2 gap-2 bg-amber-50 p-2.5 rounded-lg border border-amber-200">
                  <Field label="Hệ số hao hụt phôi (%)">
                    <NumberInput
                      allowDecimals={true}
                      value={wasteFactor}
                  ...
                  <Field label="Giá mài mặc định (đ/md)">
                    <NumberInput
              ...
              {productType !== 'service' && productType !== 'combo' && (
                <Field label="Số lượng tồn kho ban đầu">
                  <NumberInput
                    value={stockQuantity}

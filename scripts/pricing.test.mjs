...
describe('calcCartTotals', () => {
  ...
  it('P1: CK bill thổi phồng bị kẹp về tiền hàng (khớp guard server 0028)', () => {
    const t = calcCartTotals(cart({
      items: [goods()], discount_amount: 999999,
      payment_method: 'transfer', tendered_amount: 0,
    }), 500);
  ...
  it('P1: CK âm / ship âm / VAT ngoài [0,100] bị kẹp', () => {
    const t = calcCartTotals(cart({
      items: [goods()], discount_amount: -5000, shipping_fee: -3000, vat_percent: 150,
      payment_method: 'transfer', tendered_amount: 0,
    }), 500);

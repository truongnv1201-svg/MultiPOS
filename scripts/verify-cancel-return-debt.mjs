...
assert(`tạo đơn nợ ${price * 2}`, debt.ok && Number(debt.json?.debt_amount) === price * 2, debt.text);
...
assert(`thu nợ clamp ${price * 2}`, col.ok && Number(col.json?.collected) === price * 2, col.text);
...
const paid = await rpc('pos_checkout', {
  ...
  p_payments: [{ method: 'cash', amount: price }], p_note: 'verify24-cancel',
...
const paid2 = await rpc('pos_checkout', {
  p_customer_name: 'Verify 24', p_items: [item(1)], p_discount: 0,
  p_payments: [{ method: 'transfer', amount: price }], p_note: 'verify24-return',
  p_shipping_fee: 0, p_is_deposit: false,
});
const ret = await rpc('return_order_items', { p_order_id: paid2.json?.order_id, p_refund: price });
assert(`trả full: cash_refund=${price}`, ret.ok && Number(ret.json?.cash_refund) === price, ret.text);
